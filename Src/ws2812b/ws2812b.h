...
uint8_t frame_Buffer[3 * (FFT_LEN/2)];
uint8_t frame_Buffer2[3 * (FFT_LEN/2)];
...
// Public functions
// ****************
void ws2812b_init(uint8_t *,uint8_t *,uint16_t);
void ws2812b_handle();
...
typedef struct WS2812_BufferItem {
	...
	enum{READ_LOCKED,WRITE_LOCKED,BUFFER_FULL,NOT_IN_USE} buf_state;
...
WS2812_BufferItem * ws2812b_output;
